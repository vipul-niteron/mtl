...
export default function HRPage() {
  ...
  useEffect(() => {
    client.models.OvertimeRequest.list({ filter: { managerStatus: { eq: "approved" }, hrStatus: { eq: "pending" } } })
  ...
  const handleAction = async (id: string, action: "approve" | "reject") => {
    setLoading(true);
    if (action === "approve") {
      await client.models.OvertimeRequest.update({ id, hrStatus: "approved", finalStatus: "approved" });
    } else {
      await client.models.OvertimeRequest.update({ id, finalStatus: "rejected" });
    }
    // Refresh list
    const res = await client.models.OvertimeRequest.list({ filter: { managerStatus: { eq: "approved" }, hrStatus: { eq: "pending" } } });
